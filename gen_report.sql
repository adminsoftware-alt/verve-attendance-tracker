...
first_last AS (
  ...
  FROM events GROUP BY participant_name, participant_email
),
room_visits AS (
  SELECT participant_name, room, MIN(ts) as enter_time, MAX(ts) as exit_time
  FROM events WHERE event_type IN ('breakout_room_joined', 'breakout_room_left')
  GROUP BY participant_name, room
)
SELECT f.participant_name as Name, f.participant_email as Email,
  ...
  CONCAT(CAST(TIMESTAMP_DIFF(f.last_leave, f.first_join, MINUTE) / 60 AS INT64), 'h ',
    MOD(TIMESTAMP_DIFF(f.last_leave, f.first_join, MINUTE), 60), 'm') as Total_Duration,
  STRING_AGG(CONCAT(r.room, ' [', FORMAT_TIMESTAMP('%H:%M', r.enter_time, 'Asia/Kolkata'), '-',
    FORMAT_TIMESTAMP('%H:%M', r.exit_time, 'Asia/Kolkata'), ']'), ' -> ' ORDER BY r.enter_time) as Room_History
FROM first_last f LEFT JOIN room_visits r ON f.participant_name = r.participant_name
