-- ============================================================================
-- BUILD PRESENCE_INTERVALS v6 - Fixed partition range + participant_left visibility
--
-- FIXES from v5:
-- 1. Read PREVIOUS day partition too (for early morning IST events like 04:22 IST = July 21 UTC)
-- 2. Keep participant_left visible for LEAD calculation (so last intervals work)
-- 3. Filter fake joins only when part of room transition
--
...
-- ============================================================================
-- STEP 1: Get all events - READ 3 DAYS (previous, target, next) for IST coverage
-- July 22 IST = July 21 18:30 UTC to July 22 18:30 UTC
-- Early morning IST (00:00-05:30) has event_date = previous day in UTC
-- ============================================================================
WITH raw_events AS (
  SELECT
    pe.event_id,
  ...
  WHERE pe.event_date BETWEEN DATE_SUB(target_date, INTERVAL 1 DAY) AND DATE_ADD(target_date, INTERVAL 1 DAY)
...
events_with_fake_detection AS (
  SELECT
    ...
    CASE
      ...
      ELSE NULL  -- participant_left = no room (but keep for LEAD)
    END AS current_room,
    ...
    -- Detect fake join (part of room transition only)
    CASE
...
-- ============================================================================
-- STEP 6: Filter for LEAD calculation
-- Keep participant_left (for LEAD to see), but mark them so we don't generate intervals
-- Filter out: fake joins only
-- ============================================================================
events_for_lead AS (
  SELECT *
  FROM events_with_fake_detection
  WHERE NOT is_fake_join  -- Remove fake joins
    AND NOT is_fake_left  -- Remove fake lefts
    -- Keep participant_left (current_room IS NULL) - they're needed for LEAD!
),

-- ============================================================================
-- STEP 7: Compute LEAD - now participant_left is visible as end marker
-- ============================================================================
events_ordered AS (
  SELECT
    e.*,
  ...
  FROM events_for_lead e
),

-- ============================================================================
-- STEP 8: Build intervals
-- Only from events that SET a room (not participant_left)
-- participant_left is a valid "next event" for ending intervals
-- ============================================================================
intervals_raw AS (
  SELECT
    e.participant_key,
    ...
    CASE
      -- participant_left = valid end marker
      WHEN e.next_event_ts IS NOT NULL
           AND e.next_event_type IN ('participant_left', 'meeting.participant_left')
      THEN e.next_event_ts
      -- breakout events = valid continuation
      WHEN e.next_event_ts IS NOT NULL
           AND e.next_event_type IN ('breakout_room_joined', 'breakout_room_left')
      THEN e.next_event_ts
      -- participant_joined = new session, end current interval at start (0 duration)
      WHEN e.next_event_ts IS NOT NULL
           AND e.next_event_type IN ('participant_joined', 'meeting.participant_joined')
      THEN e.event_timestamp
      -- No next event → end at start (0 duration)
  ...
  WHERE e.current_room IS NOT NULL  -- Only events that SET a room (not participant_left)
),

-- ============================================================================
-- STEP 9: Apply login-date rule - only intervals STARTING on target date IST
-- ============================================================================
